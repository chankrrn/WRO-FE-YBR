...
class ImageDrawingUtils:
    ...
    @staticmethod
    def draw_label(image, text, position, color=(255, 255, 255), scale=0.6):
        """
        Draw readable text on an image.

        Below ~0.5 scale with thickness 1, antialiasing leaves the glyphs with no
        solid core - almost every stroke pixel is a partial-intensity edge pixel,
        so the text washes out to a pale smear instead of showing `color`. This
        draws at thickness 2 over a black outline so the label stays legible (and
        the right color) on both dark and bright backgrounds.
        """
        if image is None:
            return image
    ...
    def add_text_to_image(image, text, position=(50, 50), color=(255, 255, 255)):
        ...
        """
        return ImageDrawingUtils.draw_label(image, text, position, color)
